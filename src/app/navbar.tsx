...
import { Home, Wallet, LogOut, Plug, Menu, X, UserPlus } from 'lucide-react';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@/components/ui/dropdown-menu";
import { useWalletConnection } from '@/hooks/useWalletConnection';

export default function Navbar() {
    const { address, accountData, connectWallet, disconnectWallet, changeAccount } = useWalletConnection();
                ...
                {isMenuOpen && (
                    <div className="mt-4 md:hidden">
                        ...
                        <div className="mt-4 flex flex-col space-y-2">
                            {address && (
                                <Button variant="ghost" className="flex items-center justify-center space-x-2 hover:bg-gray-700">
                                    <Avatar>
                                        <AvatarFallback>{address.slice(2, 4).toUpperCase()}</AvatarFallback>
                                    </Avatar>
                            ...
                            {address ? (
                                <>
                                    <Button onClick={handleChangeAccount} className="bg-gray-800 text-white hover:bg-gray-700">
                                        Change Account
                                    </Button>
                                    <Button onClick={handleDisconnectWallet} className="bg-gray-800 text-white hover:bg-gray-700">
                                        Disconnect Wallet
                                    </Button>
                                </>
                            ) : (
                                <Button onClick={handleConnectWallet} className="bg-gray-800 text-white hover:bg-gray-700">
                                    Connect Wallet
                                </Button>
                            )}
                        </div>
